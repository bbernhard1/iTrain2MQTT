...
void z21Class::setCANDetector(uint16_t NID, uint16_t Adr, uint8_t port, uint8_t typ, uint16_t v1, uint16_t v2) {
	byte data[10];
	data[0] = NID & 0xFF;  //data[0] = NID & 0x08; // mod by bb
	data[1] = NID >> 8;   
	data[2] = Adr & 0x08;  // data[2] = Adr & 0x08; // mod by bb
	data[3] = Adr >> 8;
	data[4] = port;
	data[5] = typ;
	data[6] = v1 & 0xFF; // data[6] = v1 & 0x08;    // mod by bb
	data[7] = v1 >> 8;
	data[8] = v2 & 0xFF;  //data[8] = v2 & 0x08;    // mod by bb
	data[9] = v2 >> 8;
	EthSend(0, 0x0E, LAN_CAN_DETECTOR, data, false, Z21bcCANDetector_s);  //CAN_DETECTOR
	// EthSend(0, 0x0E, LAN_CAN_DETECTOR, data, false, Z21bcAll_s); 
}
...
void z21Class::setExtACCInfo(uint16_t Adr, byte State, byte Status) {
	byte data[5];
   ...
   if (client > 0 && BC == Z21bcNone) {
	...
	for (byte i = 0; i < z21clientMAX; i++) {
		if ( (ActIP[i].time > 0) && ( (BC & ActIP[i].BCFlag) > 0) ) {    //Boradcast & Noch aktiv
		  ...
		  if ((clientOut != client) || (clientOut == 0)) {	//wenn client > 0 und nicht Z21bcNone, sende an alle au�er den client!
